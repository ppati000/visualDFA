...
public class TaintTransition implements Transition<TaintElement> {
	...
	static class Transitioner implements StmtSwitch {
		...
		public void caseAssignStmt(AssignStmt stmt) {
			...
			boolean violated = inputElement.getValue(lValLocal).wasViolated();
			if (violated) {
				rhs.setViolated(true);
			}
	...
	static class Evaluator implements JimpleValueSwitch {
		...
		/*
		 * note that we are doing pure taint-analysis, without any notion of constants
		 * (this means that even 'y = 0 * x' taints y when x is tainted)
		 */
